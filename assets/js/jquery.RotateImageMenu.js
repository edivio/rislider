jQuery(function() {
		//our 4 items	
	var jQuerylistItems 		= jQuery('#rm_container > ul > li'),
		totalItems		= jQuerylistItems.length,
		
		//the controls
		jQueryrm_next		= jQuery('#rm_next'),
		jQueryrm_prev		= jQuery('#rm_prev'),
		jQueryrm_play		= jQuery('#rm_play'),
		jQueryrm_pause		= jQuery('#rm_pause'),
		
		//the masks and corners of the slider
		jQueryrm_mask_left	= jQuery('#rm_mask_left'),
		jQueryrm_mask_right	= jQuery('#rm_mask_right'),
		jQueryrm_corner_left	= jQuery('#rm_corner_left'),
		jQueryrm_corner_right= jQuery('#rm_corner_right'),
		
		//check if the browser is <= IE8
		ieLte8			= (jQuery.browser.msie && parseInt(jQuery.browser.version) <= 8),
			...
			var	timeDiff			= 300,
				...
				*/
				...
				initEventsHandler	= function() {
					...
					-1 	rotate left
					*/
					jQueryrm_next.bind('click', function(e) {
						stopSlideshow();
						rotateImages(1);
						return false;
					});
					jQueryrm_prev.bind('click', function(e) {
						stopSlideshow();
						rotateImages(-1);
						return false;
					});
					...
					*/
					jQueryrm_play.bind('click', function(e) {
						startSlideshow();
						return false;
					});
					jQueryrm_pause.bind('click', function(e) {
						stopSlideshow();
						return false;
					});
					/*
					...
					*/
					jQuery(document).bind('mousewheel', function(e, delta) {
						if(delta > 0) {
							stopSlideshow();
							rotateImages(0);
						}	
				...
				*/
				rotateImages		= function(dir) {
					...
					jQuerylistItems.each(function(i) {
						...
						setTimeout(function() {
							...
							var	jQueryotherImages		= jQuery('#' + jQueryitem.data('images')).children('img'),
								totalOtherImages	= jQueryotherImages.length;
								
								//the current one
								jQueryimg				= jQueryitem.children('img:last'),
								//keep track of each items current image
								current				= jQueryitem.data('current');
								//out of bounds 
								...
								var otherRotation	= (dir === 1) ? '-30deg' : '30deg',
									jQueryother			= jQueryotherImages.eq(current).clone();
								...
								jQueryitem.data('current', current).prepend(jQueryother);
								...
								if(!ieLte8) {
									...
									jQueryimg.fadeOut(1200, function(){
										jQuery(this).remove();
										++completed;
										if(completed === 4) {
											completed = 0;
				...
				configure			= function() {
					...
					jQuerylistItems.each(function(i) {
						...
						var jQueryitem = jQuery(this).data('current', 1);
						
						if(!ieLte8)
						jQueryitem.transform({rotate: jQueryitem.data('rotation') + 'deg'})
							 .find('img')
				...
				rotateMaskCorners	= function() {
					jQueryrm_mask_left.transform({rotate: '-3deg'});
					jQueryrm_mask_right.transform({rotate: '3deg'});
					jQueryrm_corner_left.transform({rotate: '45deg'});
					jQueryrm_corner_right.transform({rotate: '-45deg'});
				},
				//hides the masks and corners
				hideMaskCorners		= function() {
					jQueryrm_mask_left.hide();
					jQueryrm_mask_right.hide();
					jQueryrm_corner_left.hide();
					jQueryrm_corner_right.hide();
				},
